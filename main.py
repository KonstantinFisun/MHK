...
def show(matrix):
    x = np.linspace(matrix[0, 0] - 1, matrix[-1, 0] + 1, num=100)  # Точки по которым строится график
    ...
    for i in range(matrix.shape[0]):
        inac_lin += round(pow(matrix[i, 1] - (lin[0] * matrix[i, 0] + lin[1]), 2), 2)
        inac_sed += round(pow(matrix[i, 1] - (sed[0] * pow(matrix[i, 0], sed[1])), 2), 2)
    ...
    elif (inac_sed == min(inac_qua, inac_lin, inac_exp, inac_sed)):
        print("В данной задаче лучшей аппроксимирующей функцией является степенная функция")
    elif (inac_exp == min(inac_qua, inac_lin, inac_exp, inac_sed)):
...
def qua_func(matrix):
    ...
    m1 = np.array([[sum_xi4, sum_xi3, sum_xi2], [sum_xi3, sum_xi2, sum_xi],
                   [sum_xi2, sum_xi, matrix.shape[0]]])  # Матрица коэффициентов
